...
class MapHandler {
  ...
  private PercentAreWalls: number;
	...
	constructor(mapWidth: number, mapHeight: number, percentWalls: number = 40, map?: number[][]) {
	...
	public MakeCaverns(): void {
		...
		for(let column: number = 0, row: number = 0; row <= this.MapHeight-1; row++) {
	...
	public GetAdjacentWalls(x: number, y: number, scopeX: number, scopeY: number): number {
		...
		for(let column: number = 0, row: number = 0; row < this.MapHeight; row++) {
			for(column = 0; column < this.MapWidth; column++) {
				...
				// Else, fill with a wall a random percent of the time
					...
					this.Map.valueOf()[column][row] = row === mapMiddle ? 0 : this.RandomPercent(this.PercentAreWalls);
				}
			}
		}
	}

	public RandomPercent(percent: number): number {
		return (percent >= Math.floor(Math.random()*100)) ? 1 : 0;
	}

	private MapToString(): string {
		let returnString: string = ["Width:",
																this.MapWidth,
																"\tHeight:",
																this.MapHeight,
																"\t% Walls:",
